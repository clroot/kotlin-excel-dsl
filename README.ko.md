...
**테스트 환경**: MacBook Pro 14" M3 Pro

| 행 수 | 시간 | 메모리 (POI 렌더링) | 파일 크기 |
|-------|------|---------------------|-----------|
| 100,000 | ~1.3초 | ~2.6 MB | ~2.7 MB |
| 500,000 | ~1.3초 | - | ~10.1 MB |
| 1,000,000 | ~2.5초 | ~0.01 MB | ~20.4 MB |

**주요 특징**:
- **진정한 스트리밍**: 전체 데이터셋을 메모리에 로드하지 않고 행 단위로 처리
- **O(1) 메모리 Auto-width**: 컬럼 너비 계산 시 모든 값을 저장하지 않고 최대 너비만 추적
- **일정한 메모리 사용량**: 행 수와 무관하게 메모리 사용량이 거의 일정
